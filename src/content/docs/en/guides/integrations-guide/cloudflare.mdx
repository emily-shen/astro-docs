...
Unlike [Cloudflare Pages](#routing-on-cloudflare-pages), with Workers, you do not need a `_routes.json` file. Currently, the Cloudflare adapter always generates this file. To work around this, you should create a `.assetsignore` file in your `public/` folder, and add the following lines to it:
  ```title=".assetsignore"
  _worker.js
  _routes.json
  ```
